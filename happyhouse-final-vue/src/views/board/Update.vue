<template>
  <div>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="id" ref="item.id" v-model="item.id" readonly />
    </div>
    <!-- 제목-->
    <div class="form-group">
    ...
    <div>
      <button class="btn btn-success" @click="updateNotice" style="margin:0 10px 0 0">수정</button>
      <router-link :to="'/board/read?no=' + this.item.no">
...
export default {
  ...
  methods: {
    ...
    moveRead() {
      this.$router.push(`/board/read?no=${this.item.no}`);
    },
  },
  // 회원 정보
  created() {
